...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await saveMsrSnapshotToFirebase(propertyId, snapshot, { overwrite });
    return NextResponse.json({ ok: true, updatedAt: result.updatedAt, overwritten: result.overwritten });
  } catch (err) {
